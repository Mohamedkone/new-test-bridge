...
const getItemsByParentId = (parentId:string, structure) => {
...
const FinderApp = () => {
  ...
  // const { api, myInfo } = useContext<any>(AuthContext);
  ...
  useEffect(() => {
    const items = getItemsByParentId(currentParentId, fileSystemState)
    setCurrentItems(items);
  }, [fileSystemState, currentParentId]);

  // Navigation handlers
  const handleFolderClick = (folder: { id: React.SetStateAction<string>; name: any; }) => {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:3001/vdata`, newFolder);
